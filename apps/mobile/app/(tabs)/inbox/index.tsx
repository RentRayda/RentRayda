...
export default function InboxScreen() {
  ...
  const renderLandlordCard = ({ item }: { item: ConnectionRequest }) => (
    <View style={{ backgroundColor: '#FFFFFF', borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 }}>
      {/* Tenant info */}
      ...
      {/* Message */}
      {item.message && (
        <Text numberOfLines={2} style={{ fontSize: 14, color: '#6B7280', marginTop: 8, lineHeight: 20 }}>
          "{item.message}"
        </Text>
      )}
      ...
      {item.status === 'pending' ? (
        <View style={{ flexDirection: 'row', gap: 12, marginTop: 12 }}>
          <Pressable
            ...
            style={{ flex: 1, height: 40, backgroundColor: '#2B51E3', borderRadius: 8, alignItems: 'center', justifyContent: 'center', opacity: processing === item.id ? 0.5 : 1 }}
          >
            {processing === item.id ? (
          ...
          <Pressable
            ...
            disabled={processing === item.id}
            style={{ flex: 1, height: 40, backgroundColor: '#F3F4F6', borderRadius: 8, alignItems: 'center', justifyContent: 'center' }}
          >
        ...
        <View style={{ marginTop: 12, paddingVertical: 8, alignItems: 'center' }}>
          <Text style={{ fontSize: 14, fontWeight: '500', color: item.status === 'accepted' ? '#16A34A' : '#9CA3AF' }}>
    ...
    <View style={{ backgroundColor: '#FFFFFF', borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <View style={{
          ...
          backgroundColor: item.status === 'pending' ? '#FEF3C7' : item.status === 'accepted' ? '#D1FAE5' : '#F3F4F6',
        }}>
          <Text style={{
            fontSize: 12, fontWeight: '500',
            color: item.status === 'pending' ? '#92400E' : item.status === 'accepted' ? '#059669' : '#6B7280',
          }}>
      ...
      <Text style={{ fontSize: 14, color: '#6B7280', marginTop: 4 }}>
        ...
        {item.listing.monthlyRent ? ` · ₱${item.listing.monthlyRent.toLocaleString()}` : ''}
      </Text>
      <Text style={{ fontSize: 12, color: '#9CA3AF', marginTop: 4 }}>{timeAgo(item.createdAt)}</Text>
    </View>
  );
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: '#FAFAFA' }}>
        <View style={{ paddingHorizontal: 20, paddingTop: 16 }}>
          <Text style={{ fontSize: 20, fontWeight: '500', color: '#1A1A2E' }}>Connection Requests</Text>
        </View>
        <View style={{ paddingHorizontal: 20, paddingTop: 16 }}>
          {[1, 2, 3].map((i) => (
            <View key={i} style={{ backgroundColor: '#FFFFFF', borderRadius: 12, padding: 16, marginBottom: 12 }}>
              <View style={{ flexDirection: 'row', gap: 12, alignItems: 'center' }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={{ width: '40%', height: 12, backgroundColor: '#E5E7EB', borderRadius: 4, marginTop: 6 }} />
                </View>
              </View>
              <View style={{ width: '80%', height: 14, backgroundColor: '#E5E7EB', borderRadius: 4, marginTop: 12 }} />
              <View style={{ flexDirection: 'row', gap: 12, marginTop: 12 }}>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FAFAFA' }}>
      <View style={{ paddingHorizontal: 20, paddingTop: 16, paddingBottom: 8 }}>
        <Text style={{ fontSize: 20, fontWeight: '500', color: '#1A1A2E' }}>
          Connection Requests
        </Text>
