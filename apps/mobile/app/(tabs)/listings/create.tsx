      ...
      >
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 16 }}>
          {UNIT_TYPES.map((type) => (
            ...
            >
              <Text style={{ fontSize: 14, fontFamily: 'NotoSansOsage', color: unitType === type ? '#FFFFFF' : '#374151' }}>
                {UNIT_LABELS[type]}
              </Text>
            </Pressable>
          ))}
        ...
        <TextInput
          style={{
            ...
            paddingHorizontal: 12, fontSize: 16, fontFamily: 'NotoSansOsage', marginBottom: 16, backgroundColor: '#FFFFFF',
          }}
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
          {INCLUSIONS.map((inc) => {
            ...
            return (
              ...
              >
                <Text style={{ fontSize: 14, fontFamily: 'NotoSansOsage' }}>{inc.label}</Text>
              </Pressable>
            );
          })}
        </View>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
          <View style={{ width: 48, height: 48, borderWidth: 1, borderColor: '#CED0D4', borderRadius: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFFFFF' }}>
            <Text style={{ fontSize: 18, fontFamily: 'NotoSansOsage' }}>{beds}</Text>
          </View>
          <Pressable onPress={() => setBeds(Math.max(1, beds - 1))} disabled={beds <= 1} style={{ marginLeft: 12, width: 40, height: 40, borderRadius: 8, backgroundColor: '#CED0D4', alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ fontSize: 20, fontFamily: 'NotoSansOsage' }}>-</Text>
          </Pressable>
        ...
        <View style={{ flexDirection: 'row', gap: 24, marginBottom: 16 }}>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 14, fontFamily: 'NotoSansOsage', color: '#050505', marginBottom: 4 }}>Advance (months)</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ width: 40, height: 40, borderWidth: 1, borderColor: '#CED0D4', borderRadius: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFFFFF' }}>
                <Text style={{ fontSize: 16, fontFamily: 'NotoSansOsage' }}>{advanceMonths}</Text>
              </View>
              <Pressable onPress={() => setAdvanceMonths(Math.max(0, advanceMonths - 1))} style={{ marginLeft: 8, width: 32, height: 32, borderRadius: 6, backgroundColor: '#CED0D4', alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ fontSize: 16, fontFamily: 'NotoSansOsage' }}>-</Text>
              </Pressable>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 14, fontFamily: 'NotoSansOsage', color: '#050505', marginBottom: 4 }}>Deposit (months)</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ width: 40, height: 40, borderWidth: 1, borderColor: '#CED0D4', borderRadius: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFFFFF' }}>
                <Text style={{ fontSize: 16, fontFamily: 'NotoSansOsage' }}>{depositMonths}</Text>
              </View>
        ...
        <TextInput
          style={{
            ...
            paddingHorizontal: 12, paddingTop: 12, fontSize: 16, fontFamily: 'NotoSansOsage', marginBottom: 4,
          ...
          editable={!isLoading}
        />
        <Text style={{ fontSize: 12, fontFamily: 'NotoSansOsage', color: '#65676B', textAlign: 'right', marginBottom: 16 }}>
            ...
            {photos.map((photo, i) => (
              <Pressable key={i} onPress={() => removePhoto(i)}>
                ...
                <View style={{ position: 'absolute', top: -4, right: -4, width: 20, height: 20, borderRadius: 10, backgroundColor: '#E41E3F', alignItems: 'center', justifyContent: 'center' }}>
                  <Text style={{ color: '#FFFFFF', fontSize: 12, fontFamily: 'NotoSansOsage' }}>x</Text>
                </View>
              </Pressable>
            ))}
            {photos.length < 5 && (
              ...
              >
                <Text style={{ fontSize: 24, fontFamily: 'NotoSansOsage', color: '#8A8D91' }}>+</Text>
              </Pressable>
            )}
          </View>
        </ScrollView>
        ...
        >
          {isLoading ? (
            ...
            <Text style={{ color: '#FFFFFF', fontFamily: 'NotoSansOsage', fontSize: 16 }}>PUBLISH</Text>
          )}
        </Pressable>
      </ScrollView>
    </SafeAreaView>
