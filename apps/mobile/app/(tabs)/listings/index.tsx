  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FAFAFA' }}>
      <View style={{ paddingHorizontal: 20, paddingTop: 16, paddingBottom: 8 }}>
        <Text style={{ fontSize: 20, fontWeight: '500', color: '#1A1A2E' }}>My Listings</Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 20 }}>
          ...
          <Pressable
            ...
            style={{ marginTop: 24, height: 48, paddingHorizontal: 24, backgroundColor: '#2B51E3', borderRadius: 8, alignItems: 'center', justifyContent: 'center' }}
          >
            <Text style={{ color: '#FFFFFF', fontWeight: '600', fontSize: 16 }}>CREATE A LISTING</Text>
          </Pressable>
        </View>
      ) : (
        <>
          <FlatList
            ...
            refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchListings(); }} tintColor="#2B51E3" />}
            renderItem={({ item }) => {
              const sc = STATUS_COLORS[item.status] || STATUS_COLORS.draft;
              return (
                <Pressable
                  onPress={() => router.push(`/(tabs)/search/${item.id}` as never)}
                  style={{ backgroundColor: '#FFFFFF', borderRadius: 12, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#F3F4F6' }}
                >
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Text style={{ fontSize: 16, fontWeight: '600', color: '#2B51E3' }}>
                    ...
                    <View style={{ paddingHorizontal: 8, paddingVertical: 2, borderRadius: 9999, backgroundColor: sc.bg }}>
                      <Text style={{ fontSize: 11, fontWeight: '500', color: sc.text }}>{item.status}</Text>
                    </View>
                  </View>
          ...
          <View style={{ paddingHorizontal: 20, paddingBottom: 16 }}>
            ...
            >
              <Text style={{ color: '#FFFFFF', fontWeight: '600', fontSize: 16 }}>+ CREATE NEW LISTING</Text>
            </Pressable>
          </View>
        </>
      )}
