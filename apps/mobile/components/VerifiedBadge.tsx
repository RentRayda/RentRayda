...
export function VerifiedBadge({ status, showBpoSubBadge, size = 'md', onPress }: VerifiedBadgeProps) {
  ...
  const badge = (
    <View>
      ...
      >
        <Text style={{ fontSize: iconSize, fontFamily: 'NotoSansOsage', color: config.iconColor }}>{config.icon}</Text>
        <Text style={{ fontSize, fontFamily: 'NotoSansOsage', color: config.textColor }}>{config.label}</Text>
      </View>
      ...
      {showBpoSubBadge && status === 'verified' && (
        ...
        >
          <Text style={{ fontSize: 10, fontFamily: 'NotoSansOsage', color: '#1E40AF' }}>BPO Verified</Text>
        </View>
      )}
    </View>
  );
